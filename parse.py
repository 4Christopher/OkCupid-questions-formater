...
def printIt(que_id, pattern):
    ...
    if pattern:
        try:
            res = re.search(
                ...
                text)
            text = res.group(
                'pre') + colored(res.group('match'),
                                 'blue') + res.group('post')
        except:
            pass  # This will happen if the pattern matches at ^ and pre would be empty
    ...
    if que['data'][que_id]['isSkipped']:
        ...
        low = que['data'][que_id]['lowNumber']
        visability = 'publicly' if pub else 'privately'
        imp_text = '%d: %s, ' % (
            imp,
            importance[imp]) if show_importance else ''

        print(
            "%s (%s%s answered, on page: %s):" %
            (text, imp_text, visability, low))
        for ans_id in sorted(que['data'][que_id]['answers']):
            ...
            if match_ans and show_allowed_answer:
                sys.stdout.write(
                    colored('\t%s' %
                                (ans_text), 'green', attrs=color_attrs))
            else:
                sys.stdout.write(
                    colored('\t%s' %
                                (ans_text), attrs=color_attrs))
            if my_ans and exp:
